...
export const separarTimeBCommand = {
 data: new SlashCommandBuilder()
  .setName("separar-time-b")
 ...
 async execute(interaction: ChatInputCommandInteraction) {
  ...
  const csChannel = await guild.channels.fetch(
   config.discord.voiceCsChannelIdB
  );
  ...
  const trChannel = guild.channels.cache.get(
   config.discord.voiceQueueTrChannelIdB
  );
  const ctChannel = guild.channels.cache.get(
   config.discord.voiceQueueCtChannelIdB
  );
